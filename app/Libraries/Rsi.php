...
class Rsi
{
	public static function exec_RSI($ids, $prices)
	{
		$alpha = 1 / 14;
		return self::calc_avg_gain_loss($ids, $prices, $alpha);     
	}
	...
	private static function calc_avg_gain_loss($ids, $prices, $alpha)
	{
		...
		for ($i = 0; $i <= count($price_changes) - 1; $i++)
		{
			...
			$rsi[$ids[$i + 14]] = self::calc_RSI($avg_gain, $avg_loss);
	...
	private static function calc_RSI($avg_gain, $avg_loss)
	{
		$rsi = 0;
		if ($avg_loss == 0)
		{
